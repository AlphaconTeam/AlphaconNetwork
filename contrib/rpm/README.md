...
The RPM spec file provided here is for Alphacon-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://alphacon.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Alphacon git repository](https://github.com/AlphaconNetwork/AlphaconNetwork) but are not part of
...
Currently only `Source10` is used, for the example `alphacon.conf` file.
...
in the alphacon git repository. At present only an SVG version of the Alphacon
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the alphacon
...
needed when 0.12.1 is released, a proper fix is already in the Alphacon git
master branch.
...
idea to build Alphacon Core against a static version of that library compiled
...
The `alphacon.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `alphacon-server` package. At this point, most RPM based
...
The `alphacon-server` package should have SELinux support. How to properly do
...
Alphacon-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/alphacon/). He has been
packaging Alphacon for Fedora at least since 2012.
...
4. I renamed the `alphacon` package that contains the Qt GUI to `alphacon-core` as
that appears to be how the general population refers to it, in contrast to
`alphacon-xt` or `alphacon-classic`. I wanted to make sure the general population
