...
void CNewAsset::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(ALP_R); // r
    vchMessage.push_back(ALP_V); // v
    vchMessage.push_back(ALP_N); // n
    vchMessage.push_back(ALP_Q); // q

    vchMessage.insert(vchMessage.end(), ssAsset.begin(), ssAsset.end());
    script << OP_ALP_ASSET << ToByteVector(vchMessage) << OP_DROP;
}
...
{
    // Issuing an Asset must contain at least 3 CTxOut( Alphacon Burn Tx, Any Number of other Outputs ..., Owner Asset Tx, New Asset Tx)
    if (vout.size() < 3) {
...
{
    // Must contain at least 3 outpoints (ALP burn, owner change and one or more new unique assets that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
{
    // Reissuing an Asset must contain at least 3 CTxOut ( Alphacon Burn Tx, Any Number of other Outputs ..., Reissue Asset Tx, Owner Asset Change Tx)
    if (vout.size() < 3) {
...
bool CreateTransferAssetTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CAssetTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any ALP, transfering an asset requires a network fee"));
        return false;
    }
