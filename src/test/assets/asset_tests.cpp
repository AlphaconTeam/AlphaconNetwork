...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        BOOST_CHECK(!IsAssetNameValid("ALP", type));
        BOOST_CHECK(!IsAssetNameValid("ALPHACON", type));
        BOOST_CHECK(!IsAssetNameValid("ALPHACONCOIN", type));

        //- Versions of ALPHACONCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("ALPHACON.COIN", type));
        BOOST_CHECK(IsAssetNameValid("ALPHACON_COIN", type));
        BOOST_CHECK(IsAssetNameValid("ALPSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERALP", type));
        BOOST_CHECK(IsAssetNameValid("ALPHACONSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEALPHACON", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_ALPHACONS", type));
        BOOST_CHECK(IsAssetNameValid("SEALPOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("ALPHACON", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("ALPHACON", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
