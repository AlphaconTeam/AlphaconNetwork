...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fMemPoolCheck)
{
...
CAmount GetBlockSubsidy(int nHeight) {
    if (nHeight == Params().GetConsensus().nRewardHeighALP) {
        return Params().GetConsensus().nBlockRewardALP;
    } else {
        int halvings = nHeight / Params().GetConsensus().nBlockRewardHalvingsWindow;
        if (halvings >= Params().GetConsensus().nBlockRewardHalvings) { halvings = Params().GetConsensus().nBlockRewardHalvings; }
        CAmount nSubsidy = Params().GetConsensus().nBlockReward >> halvings;
        ...
        if (tx.IsNewAsset()) {
            ...
            if (passets->ContainsAsset(asset)) {
                if (!passets->RemoveAssetAndOutPoints(asset, strAddress)) {
                    return error("%s : Failed to Remove Asset and OutPoints. Asset Name : %s", __func__, asset.strName);
                }
            }
        }
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsNewAsset()) {
            ...
            if(!fJustCheck) {
                ...
                if (pwalletMain->IsMine(tx.vout[tx.vout.size() - 1]) == ISMINE_SPENDABLE) {
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    LogPrintf("%s: new best=%s height=%d version=%d log2_work=%.8g tx=%lu date=%s progress=%f cache=%.1fMiB(%utx)\n", __func__,
...
{
    if (block.GetBlockTime() > Params().GetConsensus().nGenesisTimestamp && block.nVersion < 7)
