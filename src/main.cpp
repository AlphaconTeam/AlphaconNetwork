...
const string strMessageMagic = "Alphacon Signed Message:\n";
...
// Get block reward
int64_t GetBlockSubsidy(int nHeight, int64_t nFees)
{
    int halvings = nHeight / Params().blockRewardHalvingsWindow;
    if (halvings >= Params().blockRewardHalvings) { halvings = Params().blockRewardHalvings; }
    int64_t nSubsidy = Params().BlockReward() >> halvings;
    LogPrint("creation", "GetBlockSubsidy(): create=%s\n", FormatMoney(nSubsidy));

    return nSubsidy + nFees;
}

// ICO Reward
int64_t GetRewardALP()
{
    int64_t nSubsidy = Params().BlockRewardALP();
    LogPrint("creation", "GetRewardALP() : create=%s\n", FormatMoney(nSubsidy));
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (Params().IsALPBlock(pindex->nHeight)) {
        if (IsProofOfWork()) {
            if (vtx[0].vout[0].scriptPubKey != GetScriptForDestination(CBitcoinAddress(Params().rewardReceiverALP).Get())) {
                return DoS(100, error("ConnectBlock() : wrong ICO reward receiver"));
            }
        } else {
            return DoS(100, error("ConnectBlock() : ICO reward block should be POW"));
        }
    } else {
        if (IsProofOfWork()) {
            int64_t nReward = GetBlockSubsidy(pindex->nHeight, nFees);
            ...
            int64_t nCalculatedStakeReward = GetBlockSubsidy(pindex->nHeight, nFees);
...
bool CBlock::AcceptBlock()
{
    ...
    if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime))
        return DoS(50, error("AcceptBlock() : coinbase timestamp is too early nTimeBlock=%d nTimeTx=%u", GetBlockTime(), FutureDrift((int64_t)vtx[0].nTime)));
