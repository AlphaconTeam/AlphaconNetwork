...
const std::string AlphaconGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...

{
    ...
    // MacDockIconHandler::instance()->setIcon(QIcon(":icons/about_alp"));
#endif
...
void AlphaconGUI::createActions()
{
    QFont font = QFont();
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and alphacon: URIs)"));
    ...
    aboutAction = new QAction(platformStyle->TextColorIcon(":/icons/about_alp"), tr("&About %1").arg(tr(PACKAGE_NAME)), this);
    ...
    toggleHideAction = new QAction(platformStyle->TextColorIcon(":/icons/about_alp"), tr("&Show / Hide"), this);
    ...
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Alphacon addresses"));
    ...
    openAction->setStatusTip(tr("Open a alphacon: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Alphacon command-line options").arg(tr(PACKAGE_NAME)));
...
void AlphaconGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
#ifdef ENABLE_WALLET
bool AlphaconGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool AlphaconGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void AlphaconGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AlphaconGUI::setWalletActionsEnabled(bool enabled)
{
...
void AlphaconGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void AlphaconGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AlphaconGUI::optionsClicked()
{
...
void AlphaconGUI::showDebugWindow()
{
...
void AlphaconGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void AlphaconGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void AlphaconGUI::openClicked()
{
...
void AlphaconGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Alphacon network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void AlphaconGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AlphaconGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void AlphaconGUI::updateHeadersSyncProgressLabel()
{
...
void AlphaconGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void AlphaconGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Alphacon"); // default title
...
void AlphaconGUI::changeEvent(QEvent *e)
{
...
void AlphaconGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AlphaconGUI::showEvent(QShowEvent *event)
{
...
void AlphaconGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AlphaconGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AlphaconGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AlphaconGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AlphaconGUI::setHDStatus(int hdEnabled)
{
...
void AlphaconGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AlphaconGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AlphaconGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void AlphaconGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void AlphaconGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(AlphaconGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void AlphaconGUI::subscribeToCoreSignals()
{
...
void AlphaconGUI::unsubscribeFromCoreSignals()
{
...
void AlphaconGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<AlphaconUnits::Unit> units = AlphaconUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AlphaconUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(AlphaconUnits::name(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(AlphaconUnits::name(u)), this);
...
{
    setText(AlphaconUnits::name(newUnits));
}
