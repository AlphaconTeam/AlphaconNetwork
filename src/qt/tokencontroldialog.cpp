    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/AlphaconNetwork/AlphaconNetwork/issues/5716
...
{
    GUIUtil::setClipboard(AlphaconUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void TokenControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = AlphaconUnits::ALP;
    ...
    l2->setText(AlphaconUnits::formatWithCustomName(QString::fromStdString(strTokenName), nTokenAmount));        // Amount
    l3->setText(AlphaconUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(AlphaconUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(AlphaconUnits::formatWithCustomName(QString::fromStdString(strTokenName), nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    for (const std::pair<QString, std::vector<COutput>> &coins : mapTokenCoins) {
        ...
        for (const COutput &out : coins.second) {
            ...
            if (ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show alphacon address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, AlphaconUnits::format(nDisplayUnit, nAmount));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, AlphaconUnits::format(nDisplayUnit, nSum));
